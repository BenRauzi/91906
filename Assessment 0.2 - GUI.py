...
def add_item_gui():
	def add_item(): #will create the item in the catalog - placed in here for scope of add_item_root, name_entry and description_entry
	...
	add_button = ttk.Button(add_item_root, text="Add Item", width=30, command=add_item)
	add_button.grid(row=1,column=0)

def view_item_details(): #a window to view the selected item's details
	def switch_item_details(item_name): #this switches the current details
		for current_item in item.items:
			if current_item.name == item_name: #break the loop to keep current_item in namespace and able to be used to set the item info
				break

		item_info = current_item.get_info()
		details_info.set(item_info)

	def exit_item_details(): #this will close the item details menu
	...
	exit_button = ttk.Button(details_frame, text="Exit", width=20, command=exit_item_details)
...
def item_management_gui(): #the window created by this function allows for the creation, removal and general overview of items in the catalog
	...
	details_button = ttk.Button(list_frame, width=39, text="View Details:")
	details_button.grid(row=1,column=0, pady=5)

	bot_frame = Frame(items_root)
	bot_frame.grid(row=1, column=0, pady=5)

	remove_button = ttk.Button(bot_frame, text="Remove Item", width=19)
