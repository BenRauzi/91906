...
notification_window = None #Same reason as above
...
def init_items(): #loads the items from the saved file on program start
	library_db = open("Library Database.ini", "r+")
	for current_item in library_db:
		item_details = current_item.split(",")

		new_item = item(item_details[0], item_details[1].strip())
		if (item_details[2].strip() == "True"): #datetime.strptime(item_details[4].strip(), '%m-%d-%Y').date()
			new_item.load_item(item_details[3].strip(), datetime.strptime(item_details[4].strip(), '%Y-%m-%d').date(),datetime.strptime(item_details[5].strip(), '%Y-%m-%d').date())
	library_db.close()
def update_items(): #updates the amount of items in the database based off the items in the current catalog
	library_db = open("Library Database.ini", "w+")
	for current_item in item.items:
		item_details = "{}, {}, {}".format(current_item.title, current_item.description, current_item.issued)
		if current_item.issued == True:
			item_details += ", {}, {}, {}\n".format(current_item.issuer_name, current_item.date_issued, current_item.date_due)
		else:
			item_details += "\n"
		library_db.write(item_details)
	library_db.close()
def remove_item(item_name): #removes an item from the catalog
	for current_item in item.items:
...
def add_item(title, description, add_item_root): #will create the item in the catalog
	...
	if len_title > 5 and len_title < 30: #boundary checks to make sure that the name is a valid length
		...
		if len_description > 10 and len_description < 100:
			...
			library_db = open("Library Database.ini", "a+") #appends the added item to the end of the saved items
			library_db.write("{}, {}, {}\n".format(title.strip(), description.strip(), False))
			library_db.close()

		else:
			error_notif("Please enter a valid brief description")
	else:
		error_notif("Please enter a valid title")
def add_item_gui(): #this gui controls adding itemss
	add_item_root = Tk()
	add_item_root.title("Add Item to Catalog")
	add_item_root.resizable(0,0)
...
def play_sound_async(sound): #plays a sound asynchronously
...
def error_notif(message): #error notification window
	global notification_window

	if not notification_window == None or message == "destroy": #this means that spamming an invalid option won't create multiple menus 
	...
	notification_window.geometry("+450+250")


	notification_label = ttk.Label(notification_window, text=message)
	notification_label.grid(row=0,column=0)

	exit_button = ttk.Button(notification_window, text="Exit", command=lambda: error_notif("destroy")) #this prevents closing the menu from causing issues when opening it again in future
	exit_button.grid(row=0,column=1)

	notification_window.protocol("WM_DELETE_WINDOW", lambda: error_notif("destroy")) #this prevents closing the menu from causing issues when opening it again in future
	
	play_sound_async("SystemExit") #this is the default Windows 10 error sound

def do_nothing(): #this function does nothing - this is used to stop the item details window from closing until the main window is closed
	pass

def view_item_details(item_id): #a window to view the selected item's details
	global item_details_root, details_info

	if item_details_root == None:
		item_details_root = Tk()
		item_details_root.title("Item Details")
		item_details_root.resizable(0,0)
		item_details_root.geometry("+610+100")
		...
		item_details_root.protocol("WM_DELETE_WINDOW", do_nothing) #prevents window closing
...
def item_management_gui(): #the window created by this function allows for the creation, removal and general overview of items in the catalog
	...
	details_button = ttk.Button(list_frame, width=39, text="View Details:", command=lambda: view_item_details(items_list))
	...
	for current_item in item.items: #adds the list of items to the listbox
		items_list.insert(0, current_item.title)
...
def main(): #this runs the main GUI
	...
	def issue_item(item_name, issuer_name, issue_days, issue_item_root): #issues the item to a person after invalid name checks. - self explanitory
		if len(issuer_name) > 5 and len(issuer_name) < 25:
			try: #checks that the number of days is a valid integer
				...
				error_notif("Please Enter a Valid number of days")
			...
			error_notif("Please enter a valid name") #eror message -- replace
	...
	def destroy_window(): #this destroys the main window and item details window
		root.destroy()
		try:
			item_details_root.destroy()
		except AttributeError: #this just prevents a little error message if you close the main window before item_details_root is declared. No real problem with it though.
			pass
	...
	def issue_item_gui():
		...
		issue_item_root.geometry("+100+100")
	...
	items_open_list.bind('<<ListboxSelect>>', lambda  event=None:view_item_details(items_open_list)) #ListboxSelect passes an event object
	...
	items_issued_list.bind('<<ListboxSelect>>', lambda  event=None:view_item_details(items_issued_list)) #ListboxSelect passes an event object
